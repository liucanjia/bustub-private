...
auto Optimizer::OptimizeSortLimitAsTopN(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::Limit) {
    ...
    BUSTUB_ASSERT(limit_plan.GetChildren().size() == 1, "Limit with multiple children?? Impossible!");

    auto child_plan = limit_plan.GetChildAt(0);
    if (child_plan->GetType() == PlanType::Sort) {
      const auto &sort_plan = dynamic_cast<const SortPlanNode &>(*child_plan);
      BUSTUB_ASSERT(sort_plan.GetChildren().size() == 1, "Sort with multiple children?? Impossible!");
